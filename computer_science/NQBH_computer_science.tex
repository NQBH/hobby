...
\paragraph{Integers.} `You can add (\texttt{+}), subtract (\texttt{-}), multiply (\texttt{*}), \& divide (\texttt{/}) integers in Python.
\begin{verbatim}
	>>> 2 + 3
	5
	>>> 3 - 2
...
\paragraph{Integers \& Floats.} ``When you divide any 2 numbers, even if they are integers that result in a whole number, you'll always get a float:
\begin{verbatim}
	>>> 4/2
	2.0
\end{verbatim}
...
\paragraph{Underscores in Numbers.} ``When you're writing long numbers, you can group digits using underscores to make large numbers more readable:
\begin{verbatim}
...
\paragraph{Multiple Assignment.} ``You can assign values to $> 1$ variable using just a single line. This can help shorten your programs \& make them easier to read; you'll use this technique most often when initializing a set of numbers. E.g., here's how you can initialize the variables $x,y,z$ to $0$:
\begin{verbatim}
	>>> x, y, z = 0, 0, 0
\end{verbatim}
