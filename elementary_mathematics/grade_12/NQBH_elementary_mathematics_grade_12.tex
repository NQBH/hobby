...
\begin{quytac}
	\begin{enumerate*}
		\item[\textbf{1.}] Tìm $f'(x)$.
		\item[\textbf{2.}] Tìm các điểm $x_i$ tại đó đạo hàm của hàm số bằng $0$ hoặc hàm số liên tục nhưng không có đạo hàm.
		\item[\textbf{3.}] Xét dấu $f'(x)$. Nếu $f'(x)$ đổi dấu khi $x$ qua điểm $x_i$ thì hàm số đạt cực trị tại $x_i$.'' -- \cite[pp. 11--14]{SGK_Toan_12_giai_tich_nang_cao}
	\end{enumerate*}
\end{quytac}
...
\begin{quytac}
	\begin{enumerate*}
		\item[\textbf{1.}] Tìm $f'(x)$.
		\item[\textbf{2.}] Tìm các nghiệm $x_i$ của phương trình $f'(x) = 0$.
		\item[\textbf{3.}] Tìm $f''(x)$ \& tính $f''(x_i)$. Nếu $f''(x_i) < 0$ thì hàm số đạt cực đại tại điểm $x_i$. Nếu $f''(x_i) > 0$ thì hàm số đạt cực tiểu tại điểm $x_i$.'' -- \cite[pp. 15--16]{SGK_Toan_12_giai_tich_nang_cao}
	\end{enumerate*}
\end{quytac}
