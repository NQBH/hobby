...
\section*{Chuỗi Bài Giảng: 1 Số Vấn Đề trong Toán Sơ Cấp -- Series: Some Topics in Elementary Mathematics}

%------------------------------------------------------------------------------%

\section*{Nguyên Tắc -- Principle}
Xem \href{https://github.com/NQBH/hobby/blob/master/elementary_mathematics/principle/NQBH_elementary_mathematics_principle.pdf}{GitHub\texttt{/}NQBH\texttt{/}elementary mathematics\texttt{/}principle}\footnote{\textsc{url}: \url{https://github.com/NQBH/hobby/blob/master/elementary_mathematics/principle/NQBH_elementary_mathematics_principle.pdf}.}.
...
\end{dinhnghia}
Nếu số dư trong phép chia $a$ cho $b$ bằng 0 thì $a$ chia hết cho $b$, ký hiệu $a\divby b$. Nếu số dư trong phép chia $a$ cho $b$ khác 0 thì $a$ không chia hết cho $b$, ký hiệu $a\not\divby b$. Với $a\in\mathbb{N}^\star$, $a$ là ước của $a$, $a$ là bội của $a$, 0 là bội của $a$, $1$ là ước của $a$.
...
Để tìm các ước của $n\in\mathbb{N}$, $n > 1$ (tức $n\ge 2$) ta có thể lần lượt chia $n$ cho các số tự nhiên từ 1 đến $n$. Khi đó, các phép chia hết cho ta số chia là ước của $n$. Ngắn gọn, tập hợp tất cả các ước của $n\in\mathbb{N}$, $n\ge 2$ là $\{a\in\mathbb{N}^\star;a\le n,\ n\divby a\}$.
...
\end{dinhly}
I.e., $(a\divby m)\land(b\divby m)\Rightarrow(a + b)\divby m$. Khi đó, $(a + b):m = a:m + b:m$, hoặc dạng phân số: $\frac{a + b}{m} = \frac{a}{m} + \frac{b}{m}$.

\begin{proof}[Proof]
	Cho $a,b,m\in\mathbb{N}$, $m\ne 0$. Nếu $a\divby m$, tồn tại $a_1\in\mathbb{N}$ sao cho $a = ma_1$. Tương tự, nếu $b\divby m$, tồn tại $b_1\in\mathbb{N}$ sao cho $b = mb_1$. Khi đó $a + b = ma_1 + mb_1 = m(a_1 + b_1)$, \textit{\&} hiển nhiên $a_1 + b_1\in\mathbb{N}$, nên $(a + b)\divby m$.
\end{proof}
...
\end{dinhly}
I.e., với $a,b,m\in\mathbb{N}$, $a\ge b$, nếu $a\divby m$ \textit{\&} $b\divby m$ thì $(a - b)\divby m$. Khi đó, ta có $(a - b):m = a:m - b:m$.

\begin{proof}[Proof]
	Cho $a,b,m\in\mathbb{N}$, $a\ge b$, $m\ne 0$. Nếu $a\divby m$, tồn tại $a_1\in\mathbb{N}$ sao cho $a = ma_1$. Tương tự, nếu $b\divby m$, tồn tại $b_1\in\mathbb{N}$ sao cho $b = mb_1$. Giả thiết $a\ge b$ cho ta $a_1\ge b_1$. Khi đó $a - b = ma_1 - mb_1 = m(a_1 - b_1)$, \textit{\&} hiển nhiên $a_1 - b_1\in\mathbb{N}$, nên $(a - b)\divby m$.
\end{proof}
...
\end{dinhly}
Nếu $a\divby m$  thì $(ab)\divby m$ với mọi $b\in\mathbb{N}$.
...
\begin{enumerate}
	\item Bất cứ số tự nhiên nào khác 0 cũng chia hết cho chính nó, i.e., $a\divby a$, $\forall a\in\mathbb{N}^\star$.
	\item \textit{Tính chất bắc cầu}: Nếu $a$ chia hết cho $b$ \& $b$ chi hết cho $c$ thì $a$ chia hết cho $c$, i.e.,
	\begin{align*}
		((a\divby b)\land(b\divby c))\Rightarrow(a\divby c),\ \forall a\in\mathbb{N},\ \forall b,c\in\mathbb{N}^\star.
	\end{align*}
	\item Số 0 chia hết cho mọi số tự nhiên $b$ khác 0, i.e., $0\divby b$, $\forall b\in\mathbb{N}^\star$.
	\item Bất cứ số tự nhiên nào cũng chia hết cho 1, i.e., $a\divby 1$, $\forall a\in\mathbb{N}$.
\end{enumerate}
...
\begin{enumerate}
	...
	\begin{align*}
		((a\divby m)\land(b\divby m))\Rightarrow(a\pm b)\divby m,\ \forall a,b\in\mathbb{N},\ \forall m\in\mathbb{N}^\star.
	\end{align*}
	
	\begin{hequa}
		Nếu tổng (hoặc hiệu) của 2 số chia hết cho $m$ \& 1 trong 2 số ấy chia hết cho $m$ thì số còn lại cũng chia hết cho $m$.
	\end{hequa}
	I.e., $(((a + b)\divby m)\land(a\divby m))\Rightarrow(b\divby m)$, $\forall a,b\in\mathbb{N}$, $\forall m\in\mathbb{N}^\star$, \& $(((a - b)\divby m)\land(a\divby m))\Rightarrow(b\divby m)$, $\forall a,b\in\mathbb{N}$, $\forall m\in\mathbb{N}^\star$, có thể ghi gộp chung 2 ý đó lại thành: $((((a + b)\divby m)\lor((a - b)\divby m))\land(a\divby m))\Rightarrow(b\divby m)$, $\forall a,b\in\mathbb{N}$, $\forall m\in\mathbb{N}^\star$
	...
	\begin{align*}
		((a\divby m)\land(b\not\divby m))\Rightarrow(a\pm b)\not\divby m,\ \forall a,b\in\mathbb{N},\ \forall m\in\mathbb{N}^\star.
	\end{align*}
\end{enumerate}
\noindent\textbf{Tính chất chia hết của tích.}
\begin{enumerate}
	\item[7.] Nếu 1 thừa số của tích chia hết cho $m$ thì tích chia hết cho $m$, i.e., $(a\divby m)\Rightarrow(ab\divby m)$, $\forall b\in\mathbb{N}$.
	\item[8.] Nếu $a$ chia hết cho $m$ \& $b$ chia hết cho $n$ thì $ab$ chia hết cho $mn$, i.e.,
	\begin{align*}
		((a\divby m)\land(b\divby n))\Rightarrow ab\divby mn,\ \forall a,b\in\mathbb{N},\ \forall m,n\in\mathbb{N}^\star.
	\end{align*}

	\begin{hequa}
		Nếu $a$ chia hết cho $b$ thì $a^n$ chia hết cho $b^n$.
	\end{hequa}
	I.e., $(a\divby b)\Rightarrow(a^n\divby b^n)$, $\forall a,n\in\mathbb{N}$, $\forall b\in\mathbb{N}^\star$.
\end{enumerate}
...
\end{dinhly}
I.e., $(a = \overline{a_na_{n-1}\ldots a_1a_0},\ n\in\mathbb{N},\ a_i\in\{0,1,2,3,4,5,6,7,9\},\,\forall i = 1,\ldots,n,\ a_0\in\{0,2,4,6,8\})\Leftrightarrow a\divby 2$.
...
\end{dinhly}
I.e., $(a = \overline{a_na_{n-1}\ldots a_1a_0},\ n\in\mathbb{N},\ a_i\in\{0,1,2,3,4,5,6,7,9\},\,\forall i = 1,\ldots,n,\ a_0\in\{0,5\})\Leftrightarrow a\divby 5$.
...
\begin{itemize}
	\item Nếu $a\in\{0;2;4;6;8\}$ thì $a\divby 2$. Do $10B\divby 2$ \textit{\&} $a\divby 2 2$ nên tổng $(10B + a)\divby 2$. Vậy $A\divby 2$. Ngược lại, nếu $A\divby 2$ thì hiệu $(A - 10B)\divby 2$, tức là $a\divby 2$ nên $a\in\{0;2;4;6;8\}$.
	\item Nếu $a\in\{0;5\}$ thì $a\divby 5$. Do $10B\divby 5$ \textit{\&} $a\divby 5$ nên tổng $(10B + a)\divby 5$. Vậy $A\divby 5$. Ngược lại, nếu $A\divby 5$ thì hiệu $(A - 10B)\divby 5$, tức là $a\divby 5$ nên $a\in\{0;5\}$.
\end{itemize}

\subsubsection{Dấu hiệu chia hết cho 4}
Tham khảo \cite[p. 37]{SGK_Toan_6_Canh_Dieu_tap_1}. Xét số tự nhiên $A$ có 3 chữ số trở lên\footnote{Giả thiết này sẽ làm thiếu những số có 1 hoặc 2 chữ số \textit{\&} chia hết cho 4.}. Gọi $C$ là số tại bởi 2 chữ số tận cùng của $A$. Khi đó $A$ có thể được viết dưới dạng: $A = 100B + C$, trong đó $B\in\mathbb{N}$ (trường hợp $B = 0$ thì $A = C$ \textit{\&} là số có 2 chữ số). Do đó, $A - 100B = C$. Nếu $C\divby 4$ thì tổng $(100B + C)\divby 4$, tức là $A\divby 4$. Ngược lại, nếu $A\divby 4$ thì hiệu $(A - 100B)\divby 4$, tức là $C\divby 4$. Vậy:

\begin{dinhly}
	Các số có 2 chữ số tận cùng tạo thành 1 số chia hết cho 4 thì chia hết cho 4 \textit{\&} chỉ những số đó mới chia hết cho 4.
\end{dinhly}
I.e., $(a = \overline{a_na_{n-1}\ldots a_1a_0},\ n\in\mathbb{N},\ a_i\in\{0,1,2,3,4,5,6,7,9\},\,\forall i = 1,\ldots,n,\ \overline{a_1a_0}\divby 4)\Leftrightarrow a\divby 4$. Chú ý công thức này bao gồm cả những số có 1 hoặc 2 chữ số \textit{\&} chia hết 4.
...
\begin{proof}[Proof]
	...
	\end{align*}
	Vì vậy, $a\divby 3$ khi \textit{\&} chỉ khi tổng các chữ số của $a$, i.e., $\sum_{i=0}^n a_i$, chia hết cho 3.
\end{proof}
...
\begin{proof}[Proof]
	...
	\end{align*}
	Vì vậy, $a\divby 9$ khi \textit{\&} chỉ khi tổng các chữ số của $a$, i.e., $\sum_{i=0}^n a_i$, chia hết cho 9.
\end{proof}
...
\begin{itemize}
	\item Nếu $A\in\mathbb{N}$ có tổng các chữ số chia hết cho 3 thì $S$ chia hết cho 3. Do $9M\divby 3$ \textit{\&} $S\divby 3$ nên tổng $(9M + S)\divby 3$. Vậy $A\divby 3$. Ngược lại, nếu $A\divby 3$ thì hiệu $(A - 9M)\divby 3$, i.e., $S\divby 3$. Vậy tổng các chữ số của $A$ chia hết cho 3.
	\item Nếu $A\in\mathbb{N}$ có tổng các chữ số chia hết cho 9 thì $S$ chia hết cho 9. Do $9M\divby 9$ \textit{\&} $S\divby 9$ nên tổng $(9M + S)\divby 9$. Vậy $A\divby 9$. Ngược lại, nếu $A\divby 9$  thì hiệu $(A - 9M)\divby 9$, i.e., $S\divby 9$. Vậy tổng các chữ số của $A$ chia hết cho 9.
\end{itemize}
...
\begin{align*}
	A\divby 2&\Leftrightarrow a_0\divby 2,\ A\divby 5\Leftrightarrow a_0\divby 5,\\
	A\divby 4&\Leftrightarrow\overline{a_1a_0}\divby 4,\ A\divby 25\Leftrightarrow\overline{a_1a_0}\divby 25,\\
	A\divby 8&\Leftrightarrow\overline{a_2a_1a_0}\divby 8,\ A\divby 125\Leftrightarrow\overline{a_2a_1a_0}\divby 125,\\
	A\divby 3&\Leftrightarrow a_n + a_{n-1} + \cdots + a_1 + a_0 = \sum_{i=0}^n a_i\divby 3,\\
	A\divby 9&\Leftrightarrow a_n + a_{n-1} + \cdots + a_1 + a_0 = \sum_{i=0}^n a_i\divby 9.
\end{align*}
...
``Phép chia chỉ thực hiện được trong phạm vi số nguyên khi số bị chia chia hết cho số chia. Trong trường hợp $a\divby b$, ta nói: $a$ là bội của $b$ \& $b$ là ước của $a$. Ước chung (hoặc bội chung) của 2 hay nhiều số là ước (hoặc bội) của tất cả các số đó.'' -- \cite[p. 32]{Binh_Toan_6_tap_1}
...
\section{Một Số Yếu Tố Thống kê \textit{\&} Xác Suất}
...
\subsection{Thu Thập, Tổ Chức, Biểu Diễn, Phân Tích, \textit{\&} Xử Lý Dữ Liệu}
Những bước chính trong tiến trình thống kê: ``thu thập, phân loại, kiểm đếm, ghi chép số liệu; đọc \textit{\&} mô tả các số liệu ở dạng dãy số liệu, bảng số liệu hoặc ở dạng biểu đồ (biểu đồ tranh, biểu đồ cột hoặc biểu đồ hình quạt tròn); nêu được nhận xét đơn giản từ biểu đồ.'' -- \cite[p. 3]{SGK_Toan_6_Canh_Dieu_tap_2}.

\subsubsection{Thu thập, tổ chức, phân tích, \textit{\&} xử lý dữ liệu}
``Sau khi thu thập, tổ chức, phân loại, biểu diễn dữ liệu bằng bảng hoặc biểu đồ, ta cần phân tích \textit{\&} xử lý các dữ liệu đó để tìm ra những thông tin hữu ích \textit{\&} rút ra kết luận.'' [$\ldots$] ``Ta có thể nhận biết được tính hợp lý của dữ liệu thống kê theo những tiêu chí đơn giản.'' [$\ldots$] ``Dựa theo đối tượng \textit{\&} tiêu chí thống kê, ta có thể tổ chức \textit{\&} phân loại dữ liệu.'' -- \cite[p. 4]{SGK_Toan_6_Canh_Dieu_tap_2}.

``Dựa vào thống kê, ta có thể nhận biết được tính hợp lý của kết luận đã nêu ra.'' -- \cite[p. 5]{SGK_Toan_6_Canh_Dieu_tap_2}.
...
2 mặt của đồng xu: mặt sấp\texttt{/}S hay mặt ngửa\texttt{/}N. Khi tung đồng xu 1 lần, có 2 kết quả có thể xảy ra đối với mặt xuất hiện của đồng xu, đó là: mặt N; mặt S. Có 2 điều cần chú ý trong mô hình xác suất của trò chơi tung đồng xu:
\begin{itemize}
...
``Người ta chứng minh được rằng khi số lần tung càng lớn thì xác suất thực nghiệm xuất hiện mặt N càng gần với 0.5. Số 0.5 được gọi là \textit{xác suất xuất hiện mặt N} (theo nghĩa thống kê).'' -- \cite[p. 21]{SGK_Toan_6_Canh_Dieu_tap_2}. Phương pháp tung kim để tính số $\pi$ của Bá tước Georges-Louis Leclerc de Buffon chính là tiền thân của phương pháp Monte--Carlo trong toán học.
...
\begin{luuy}
	Có vô số điểm thuộc 1 đoạn\emph{\texttt{/}}đường thẳng.
\end{luuy}
